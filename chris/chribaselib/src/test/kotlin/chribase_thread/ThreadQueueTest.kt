...
class ThreadQueueTest {
     ...
     * released. Check that in the end 2*chribase.DEFAULT_MAX_THREAD_QUEUE messages have come through the queue.
     */
    @Test fun fillFreeQueueWithCongestion () {
        val thread = object : CuteThread(0) {
            override fun run() {
                Thread.sleep(1000)
                var i = 0
                while(true) {
