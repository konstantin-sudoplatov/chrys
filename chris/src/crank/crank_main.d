...
enum Chat: DcpDescriptor {
    ...
    activateRemotely_readyForUlineInput_chat_c2act = cd!(SpA_CidCid, 3_702_223_557),
...
void chatBranch() {
    ...
    cp!sendUlineChatBreed_chat_c2act.load(statCid!sendConceptToBranch_stat, uline_breed, chatBreed_hardcid_breed);
    cp!sendUlineUserTid_chat_c2act.load(statCid!sendConceptToBranch_stat, uline_breed, userThread_hardcid_tidprem);
    cp!activateRemotely_readyForUlineInput_chat_c2act.load(statCid!activateRemotely_stat, uline_breed,
            chatReadyForUlineInputPeg_uline_pegprem);
    cp!shakeHandsWithUline_chat_actnrn.addEffects(
        [   // acts
            sendUlineChatBreed_chat_c2act,       // give uline own breed
            sendUlineUserTid_chat_c2act,         // give uline user's Tid
            activateRemotely_readyForUlineInput_chat_c2act,    // tell uline to send next line
        ],
    ...
    cp!valveOnUlineInput_chat_andnrn.addEffects(
        ...
        [
            ...
            activateRemotely_readyForUlineInput_chat_c2act
        ],
        null
    );
}
...
enum Uline {
    ...
    sendUserUlineTid_uline_c2act = cd!(SpA_CidCid, 2_277_726_710),
    ...
    moveLineFromUserInuputBufferToUserInputLine_uline_c2act = cd!(SpA_CidCid, 2_949_480_003),

    /// Send user line premise to chat (together with the activation value)
    sendUserInputLineToChat_uline_c2act = cd!(SpA_CidCid, 3_447_310_214),
...
void ulineBranch() {
    ...
    cp!sendUserUlineTid_uline_c2act.load(statCid!sendTidToUser_stat, userThread_hardcid_tidprem, uline_breed);
    ...
    cp!userInputValve_uline_andnrn.addPremises([
        ...
        // Actions
    cp!moveLineFromUserInuputBufferToUserInputLine_uline_c2act.load(statCid!getUserInputLineFromBuffer_stat,
            userInputBuffer_hardcid_strqprem, userInputLine_strprem);
    cp!sendUserInputLineToChat_uline_c2act.load(statCid!sendConceptToBranch_stat, chatBreed_hardcid_breed, userInputLine_strprem);
    ...
    cp!userInputValve_uline_andnrn.addEffects(
        float.infinity,
        [
            moveLineFromUserInuputBufferToUserInputLine_uline_c2act,
            sendUserInputLineToChat_uline_c2act,
